...
function updateButtons(opts) {
  ...
  if (nstore$1 <= current) $(id + " .fa-redo").addClass("disabled");
...
async function save$1(opts) {
  try {
    ...
    console.log("hellooo");
        ...
        const csv = content; // assuming this is the CSV data you want to insert
        const img = opts.img; // change this based on how you're getting the image
    ...
    console.log(userId);
        ...
        const response = await fetch('project/db/family_pedigree.php', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({ csv, img, userId }), // sending the data to PHP
        });
...
function addparents(opts, dataset, name) {
  ...
  if (depth === 1) {
    ...
    for (i = 0; i < dataset.length; i++) {
      if (
        (dataset[i].top_level ||
          getDepth(dataset, dataset[i].name) === 2) &&
    ...
    else midx = getIdxByName(dataset, node.mother);
...
function delete_node_dataset(dataset, node, opts, onDone) {
  ...
  if (node.parent_node) {
    for (i = 0; i < node.parent_node.length; i++) {
      ...
      let ps = [
        getNodeByName(dataset, parent.mother.name),
    ...
    if (sibs.length < 1) {
      ...
      for (j = 0; j < ancestors.length; j++) {
        ...
        if (ancestors[j].data.mother) {
          ...
          dataset.splice(
            getIdxByName(dataset, ancestors[j].data.mother.name),
            1
          );
          dataset.splice(
            getIdxByName(dataset, ancestors[j].data.father.name),
            1
          );
        }
      }
